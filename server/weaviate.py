...
def _get_client() -> WeaviateClient:
    return connect_to_local(
        host=getenv("WEAVIATE_HOST"), # pyright: ignore # Assume this variable has been set corretly
        port=int(getenv("WEAVIATE_PORT")) # pyright: ignore
    )
...
def vector_search(vector: list[int], count: int = 5) -> list[str]:
    ...
    with _get_client() as client:
        ...
        for result in results.objects:
            sites.append(result.properties['url']) # pyright: ignore # This is definitely a string
